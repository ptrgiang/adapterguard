...
**Stable:** `v0.4.1`  
**Development:** `main` / `0.5.0` — verification coverage policy, chat completions, real HF smoke,
and tokenizer/chat-template integrity.
...
All three proofs are exercised by the **HF Semantic Smoke** workflow on relevant `main` changes and
...
- chat-template rendering and chat token-ID equivalence when verifying a chat endpoint
...
- OpenAI-compatible single-turn `/v1/chat/completions`
...
Single-turn chat completions on v0.5 `main`:
...
For chat verification, each JSONL `prompt` becomes one user message. AdapterGuard applies the base
tokenizer's configured chat template locally with `add_generation_prompt=true`, then compares that
reference with the served response. A missing chat template fails explicitly instead of silently
...
- OpenAI-compatible single-turn chat completions
...
loading, native multi-turn `messages` JSONL, TGI-specific verification, and runtime-to-runtime drift
matrices.
